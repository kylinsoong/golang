...
import (
	...
	"github.com/kylinsoong/bigip-ctlr/pkg/agent/cccl"
...
func _init() {
	...
	// Global flags
	pythonBaseDir = globalFlags.String("python-basedir", "", "DEPRECATED: Optional, directory location of python utilities")
	logLevel = globalFlags.String("log-level", "INFO", "Optional, logging level")
	ccclLogLevel = globalFlags.String("cccl-log-level", "", "Optional, logging level for cccl")
	verifyInterval = globalFlags.Int("verify-interval", 30, "Optional, interval (in seconds) at which to verify the BIG-IP configuration.")
	...
	httpAddress = globalFlags.String("http-listen-address", "0.0.0.0:8080", "Optional, address to serve http based informations (/metrics and /health).")
	disableTeems = globalFlags.Bool("disable-teems", false, "Optional, flag to disable sending telemetry data to TEEM")
	// Custom Resource
	enableIPV6 = globalFlags.Bool("enable-ipv6", false, "Optional, flag to enbale ipv6 network support.")
	customResourceMode = globalFlags.Bool("custom-resource-mode", false, "Optional, When set to true, controller processes only F5 Custom Resources.")
	controllerMode = globalFlags.String("controller-mode", "", "Optional, to put the controller to process desired resources.")
	defaultRouteDomain = globalFlags.Int("default-route-domain", 0, "Optional, CIS uses this value as default Route Domain in BIG-IP ")
	...
	// BigIP flags
	bigIPURL = bigIPFlags.String("bigip-url", "", "Required, URL for the Big-IP")
	bigIPUsername = bigIPFlags.String("bigip-username", "", "Required, user name for the Big-IP user account.")
	bigIPPassword = bigIPFlags.String("bigip-password", "", "Required, password for the Big-IP user account.")
	bigIPPartitions = bigIPFlags.StringArray("bigip-partition", []string{}, "Required, partition(s) for the Big-IP kubernetes objects.")
	credsDir = bigIPFlags.String("credentials-directory", "", "Optional, directory that contains the BIG-IP username, password, and/or url files. To be used instead of username, password, and/or url arguments.")
	as3Validation = bigIPFlags.Bool("as3-validation", true, "Optional, when set to false, disables as3 template validation on the controller.")
	sslInsecure = bigIPFlags.Bool("insecure", false, "Optional, when set to true, enable insecure SSL communication to BIGIP.")
	ipam = bigIPFlags.Bool("ipam", false, "Optional, when set to true, enable ipam feature for CRD.")
	as3PostDelay = bigIPFlags.Int("as3-post-delay", 0, "Optional, time (in seconds) that CIS waits to post the available AS3 declaration.")
	...
	shareNodes = bigIPFlags.Bool("share-nodes", false, "Optional, when set to true, node will be shared among partition.")
	enableTLS = bigIPFlags.String("tls-version", "1.2", "Optional, Configure TLS version to be enabled on BIG-IP. TLS1.3 is only supported in tmos version 14.0+.")
	tls13CipherGroupReference = bigIPFlags.String("cipher-group", "/Common/f5-default", "Optional, Configures a Cipher Group in BIG-IP and reference it here. cipher-group and ciphers are mutually exclusive, only use one.")
	...
	trustedCertsCfgmap = bigIPFlags.String("trusted-certs-cfgmap", "", "Optional, when certificates are provided, adds them to controller'trusted certificate store.")
	...
	overrideAS3UsageStr := "Optional, provide Namespace and Name of that ConfigMap as <namespace>/<configmap-name>. The JSON key/values from this ConfigMap will override key/values from internally generated AS3 declaration."
	...
	manageRoutes = kubeFlags.Bool("manage-routes", false, "Optional, specify whether or not to manage Route resources")
	...
	resolveIngNames = kubeFlags.String("resolve-ingress-names", "", "Optional, direct the controller to resolve host names in Ingresses into IP addresses. The 'LOOKUP' option will use the controller's built-in DNS. Any other string will be used as a custom DNS server, either by name or IP address.")
	defaultIngIP = kubeFlags.String("default-ingress-ip", "", "Optional, the controller will configure a virtual server with this IP address for any Ingress with the annotation 'virtual-server.f5.com/ip:controller-default'.")
	vsSnatPoolName = kubeFlags.String("vs-snat-pool-name", "", "Optional, the controller will configure each virtual server to reference the pool with this name.")
	useSecrets = kubeFlags.Bool("use-secrets", true, "Optional, enable/disable use of Secrets for Ingress or ConfigMap SSL Profiles.")
	schemaLocal = kubeFlags.String("schema-db-base-dir", "file:///app/vendor/src/f5/schemas/", "Optional, where the schema db's locally reside")
	// TODO once ingress extentionv1/beta1 api is deprecated we can remove this deployment parameter
	manageIngressClassOnly = kubeFlags.Bool("manage-ingress-class-only", false, "Optional, default `false`. Process all ingress resources without `kubernetes.io/ingress.class` annotation and ingresses with annotation `kubernetes.io/ingress.class=f5`.")
	ingressClass = kubeFlags.String("ingress-class", "f5", "Optional, default `f5`. A class of the Ingress controller. The Ingress controller only processes Ingress resources that belong to its class - i.e. have the annotation `kubernetes.io/ingress.class` equal to the class. Additionally, the Ingress controller processes Ingress resources that do not have that annotation, which can be disabled by setting the `-manage-ingress-class-only` flag")
	...
	vxlanMode = ""
	openshiftSDNName = vxlanFlags.String("openshift-sdn-name", "", "Must be provided for BigIP SDN integration, full path of BigIP OpenShift SDN VxLAN Tunnel")
	flannelName = vxlanFlags.String("flannel-name", "", "Must be provided for BigIP Flannel integration, full path of BigIP Flannel VxLAN Tunnel")
	...
	routeVserverAddr = osRouteFlags.String("route-vserver-addr", "", "Optional, bind address for virtual server for Route objects.")
	routeLabel = osRouteFlags.String("route-label", "", "Optional, label for which Route objects to watch.")
	routeHttpVs = osRouteFlags.String("route-http-vserver", "ose-vserver", "Optional, the name to be used for the OpenShift Route http vserver")
	routeHttpsVs = osRouteFlags.String("route-https-vserver", "https-ose-vserver", "Optional, the name to be used for the OpenShift Route https vserver")
	clientSSL = osRouteFlags.String("default-client-ssl", "", "Optional, specify a user-created client ssl profile to be used as default for SNI for Route virtual servers")
	serverSSL = osRouteFlags.String("default-server-ssl", "", "Optional, specify a user-created server ssl profile to be used as default for SNI for Route virtual servers")
	routeSpecConfigmap = osRouteFlags.String("route-spec-configmap", "", "Required, specify a configmap that holds additional spec for routes if controller-mode is 'openshift'")
	...
	// GTM Big IP flags
	gtmBigIPURL = gtmBigIPFlags.String("gtm-bigip-url", "", "Optional, URL for the GTM Big-IP")
	gtmBigIPUsername = gtmBigIPFlags.String("gtm-bigip-username", "", "Optional, user name for the GTM Big-IP user account.")
	gtmBigIPPassword = gtmBigIPFlags.String("gtm-bigip-password", "", "Optional, password for the GMT Big-IP user account.")
	gtmCredsDir = gtmBigIPFlags.String("gtm-credentials-directory", "", "Optional, directory that contains the GTM BIG-IP username, password, and/or url files. To be used instead of username, password, and/or url arguments.")
...
func verifyArgs() error {
	*logLevel = strings.ToUpper(*logLevel)
	...
	if flags.Changed("openshift-sdn-name") {
		...
		if isNodePort {
			return fmt.Errorf("Cannot run NodePort mode while supplying openshift-sdn-name. Must be in Cluster mode if using VXLAN.")
		}
		vxlanMode = "maintain"
		...
		if isNodePort {
			return fmt.Errorf("Cannot run NodePort mode while supplying flannel-name. Must be in Cluster mode if using VXLAN.")
		}
		vxlanMode = "maintain"
	...
	if *overriderAS3CfgmapName != "" {
		if len(strings.Split(*overriderAS3CfgmapName, "/")) != 2 {
			return fmt.Errorf("Invalid value provided for --override-as3-declaration, Usage: --override-as3-declaration=<namespace>/<configmap-name>")
		}
	}
...
func getCredentials() error {
	if len(*credsDir) > 0 {
		...
		setField := func(field *string, filename, fieldType string) error {
			...
			if readErr != nil {
				log.Debug(fmt.Sprintf("No %s in credentials directory, falling back to CLI argument", fieldType))
				if len(*field) == 0 {
...
func getGTMCredentials() {
	...
	if len(u.Path) > 0 && u.Path != "/" {
		log.Errorf("GTM BIGIP-URL path must be empty or '/'; check URL formatting and/or remove %s from path", u.Path)
	}
}
...
func setupNodePolling(
	...
	if 0 != len(vxlanMode) {
		...
		if nil != err {
			return fmt.Errorf("error registering node update listener for vxlan mode: %v", err)
		}
		if eventChanl != nil {
