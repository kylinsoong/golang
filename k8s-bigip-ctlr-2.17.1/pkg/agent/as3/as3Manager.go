...
func (am *AS3Manager) postAS3Declaration(rsReq ResourceRequest) (bool, string, error) {
	log.Debug("[TEST] Posting AS3 declaration start")
...
func (am *AS3Manager) ConfigDeployer() {
	...
	for msgReq := range am.ReqChan {
		...
		log.Debugf("[TEST] --> Receive message from %v, ReqID: %d, MsgType: %s, ResourceRequest: %v", am.ReqChan, msgReq.ReqID, msgReq.MsgType, msgReq.ResourceRequest.AgentCfgmaps)
